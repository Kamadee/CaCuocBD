...
class MatchController extends Controller
{
    ...
    public function postEditMatch(Request $request)
    {
        ...
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'away_team' => ['required'],
                'startMatch' => 'required|date',
                'endMatch' => 'required|date',
                'betting_odds' => 'required|numeric|min:0',
                'betting_odds_stop' => 'required|date|before:startMatch',
                'isPublic' => Rule::in(['1']),
            ]);
            if ($validator->fails()) {
                ...
                $match = MatchModel::find($matchId);
                /* Update money for user who betted in current match */
                if ($match->result) {
                    ...
                    $bettingOds = $match->betting_odds;
                    foreach ($userBettingList as $userBetting) {
                        $userChoice = $userBetting->choice;
                        $moneyBet = $userBetting->money_bet;
                        if ($userChoice == $homeClubResult) {
                            /* Cong tien */
                            $moneyWin = $moneyBet * $bettingOds;
    ...
    public function getUserBettingList(Request $request)
    {
        ...
        return view('match.userBettingList', ['match' => $match, 'choiceList' => $choiceList]);
    }
